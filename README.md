...
- Reference link definitions, abbreviation definitions, and HTML comments are parsed as visible, inspectable structures, with references and reference images resolved by CommonMark first-wins semantics plus duplicate-definition diagnostics, and abbreviations resolved in preview and HTML export.
...
- Editor focus feedback with current-line highlight and richer Markdown syntax coloring for YAML/TOML front matter, range-aware fenced code spans, exact-run multiline inline code spans with protected contents, hard line breaks, HTML entities, bold, italic, highlights, superscript, subscript, CriticMarkup review marks, citations, emoji shortcodes, keyboard tags, inline HTML tags, images, links, footnotes, URLs, emails, tables, comments, and more.
...
- Live diagnostics for empty files, unclosed front matter, variable-length code fences, math blocks, missing or duplicate link/image references, missing footnotes, duplicate explicit heading anchors, and duplicate block ids.
