...
The editor keeps `NSTextView` text authoritative while it is first responder, so SwiftUI state updates cannot overwrite in-progress typing or marked IME text. Document text still updates for saving, but preview parsing, inspector analysis, active preview-line updates, and editor syntax repainting stay paused during active input; Save/Command-S or Command-R performs one explicit refresh from the current Markdown source. This keeps split-view preview, inspector, status, and highlighting work from stealing selection, scroll position, or keystroke time. Status-bar messages report paused, refreshing, saved, and updated preview states, with successful transient messages clearing automatically so the bar does not go stale.
