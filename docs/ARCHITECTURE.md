...
The editor keeps `NSTextView` text authoritative while it is first responder, so SwiftUI state updates cannot overwrite in-progress typing or marked IME text. Document text still updates for saving, but preview parsing pauses during active input and resumes after a short idle window; this keeps split-view preview, inspector, and status updates from stealing selection or scroll position. Command-R cancels pending parse work and immediately refreshes the preview, inspector, and status model from the current Markdown source. Status-bar messages report paused, refreshing, and updated preview states, with successful transient messages clearing automatically so the bar does not go stale.
