...
Inline parsing is backed by a bounded, thread-safe cache. Preview redraws, marker extraction, and export often revisit identical inline strings, so the cache cuts repeated tokenization while skipping very large text runs to keep memory predictable. Marker extraction streams through each inline-capable block and classifies parsed segments immediately, avoiding broad intermediate arrays when long documents contain many links, tags, references, review marks, and symbol tokens. Explicit heading anchors are preserved on heading blocks during parsing, so marker analysis can report custom anchors without re-splitting the full Markdown source into lines. Document text stats count words, characters, lines, and reading time in one pass instead of splitting the whole source into temporary arrays. Reference definitions use CommonMark first-wins semantics across preview, export, and diagnostics, support escaped label brackets plus destination and title continuations on following lines, unescape CommonMark backslash escapes in destinations and titles, require balanced unescaped parentheses in bare destinations, reject invalid labels, incomplete split definitions, or trailing title text instead of silently dropping it, and surface duplicate normalized labels as informational diagnostics. Missing-reference and missing-footnote diagnostics share one stream over inline-capable block surfaces instead of scanning raw source regexes or parsing the same text twice, so collapsed links/images and footnote references are covered while code spans and non-inline blocks are ignored; shortcut references remain context-aware candidates and only resolve when a matching definition exists.

Preview rendering avoids allocating temporary enumerated arrays for block, list, table, and definition-list loops. Source-map line lookups use binary search when mapping the editor caret or line-navigation requests back to a rendered block. Editor-driven preview sync also coalesces rapid caret changes with a short cancellable delay, while explicit link navigation remains immediate. Large previews automatically suppress update pulses, block-count animations, active-block animation, and animated scrolls so explicit refreshes do less main-thread visual work.

The editor keeps `NSTextView` text authoritative while it is first responder, so SwiftUI state updates cannot overwrite in-progress typing or marked IME text. During active input the editor updates only its local text storage and flips a lightweight dirty marker once when the draft first changes; it does not write the SwiftUI document binding, parse Markdown, update the inspector, move preview focus, or repaint syntax. The stale preview is explicitly marked as paused and does not accept preview-side task edits while the draft is dirty. Save/Command-S or Command-R reads the local draft once, commits it to the document model, ignores the resulting self-authored document-change echo, and performs one explicit refresh from that Markdown source. If typing resumes while that refresh is still parsing, the returning parse result is ignored unless the current local draft still matches the parsed text, so the preview cannot unfreeze around a stale snapshot. `MarkdownDocument` also asks the draft registry for the latest text while writing, so system autosaves can persist the current editor draft without per-keystroke document updates or UI refreshes. Status-bar messages report paused, refreshing, saved, and updated preview states, with successful transient messages clearing automatically so the bar does not go stale.
...
Editor syntax highlighting keeps fenced code lines and their contents as protected ranges through `MarkdownCodeFenceScanner`, so inline passes do not recolor backticks or Markdown-looking text inside code fences. Display math blocks use `MarkdownMathBlockScanner` for the same protected-range treatment across `$$...$$` and `\[`/`\]` forms, preventing math content from being recolored as emphasis, links, or inline code. Inline code styling uses `MarkdownCodeSpanScanner` to require equal-length backtick runs, including CommonMark code spans that cross soft line breaks, while skipping true fence delimiter lines so `` ```text `` stays a fenced block marker. The text view's backtick smart-pairing is also fence-aware, letting line-start triple backticks stay literal while ordinary inline code still gets paired. The shared fence scanner also treats smart single-quote triples such as `‘’‘text` and `’‘’` as backtick-compatible fences, recovering code blocks from smart punctuation or pasted text before inline coloring runs. `MarkdownSyntaxHighlightScanner` merges fenced ranges, display math ranges, and inline code spans into the exclusion ranges used by later inline highlighters, so text like `` `**not bold**` `` or `$$ **not bold** $$` cannot be recolored by emphasis, link, tag, or TODO passes. For large documents, the editor asks the scanners only for ranges intersecting the active highlight window, clips returned protected ranges to that same window before AppKit applies attributes, and resolves overlapping protected blocks by source order.
...
- Thematic breaks that terminate paragraphs while preserving four-space indented marker lines as code.
...
- Fenced code blocks with variable-length backtick or tilde delimiters, smart single-quote triples normalized as backtick-compatible fences, CommonMark's zero-to-three-space indentation, content de-indentation, backtick info-string validation, and first-word language extraction rules, plus indented code blocks.
- Diagram fences for Mermaid, Graphviz, PlantUML, sequence, and flowchart sources.
- Inline emphasis, strong text, strikethrough, highlights, superscript, subscript, CriticMarkup review marks, CommonMark code spans with multi-backtick delimiters and spacing normalization, CommonMark hard line breaks, HTML entity references, links and images with balanced parentheses, backslash-unescaped destinations/titles, and nested brackets in labels, full, collapsed, and shortcut reference links/images, citations, resolved emoji shortcodes, keyboard tags, inline HTML tags, raw and `www.` autolinks with balanced parentheses, CommonMark angle-bracket autolinks, email links, wiki links and embeds with aliases and heading/block targets, tags, mentions, footnotes, images, Markdown Extra abbreviations, and math markers.
- Block and inline math markers.
- Ordered lists with both `1.` and `1)` markers, preserved ordered-list start numbers, lazy continuation lines for list and task items, GFM-style tables including escaped pipes and code-span pipes inside cells, plus task lists with extended states such as `[/]`, `[-]`, `[>]`, `[!]`, and `[?]`.
