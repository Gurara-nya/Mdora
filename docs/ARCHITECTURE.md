...
Paste normalization is kept in `MarkdownPasteTransformer`; the AppKit text view only reads the pasteboard and applies a replacement when a single URL can safely become a Markdown link or image. Local image files resolve relative to the current Markdown document when possible, keeping pasted image references portable across folders.
...
- GFM-style tables and task lists, with extended task states such as `[/]`, `[-]`, `[>]`, `[!]`, and `[?]`.
