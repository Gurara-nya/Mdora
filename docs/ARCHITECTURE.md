...
Inline parsing is backed by a bounded, thread-safe cache. Preview redraws, marker extraction, and export often revisit identical inline strings, so the cache cuts repeated tokenization while skipping very large text runs to keep memory predictable. Marker extraction then walks the parsed inline segment stream once for most inline marker families, avoiding repeated scans when long documents contain many links, tags, references, review marks, and symbol tokens. Reference definitions use CommonMark first-wins semantics across preview, export, and diagnostics, support escaped label brackets plus destination and title continuations on following lines, unescape CommonMark backslash escapes in destinations and titles, require balanced unescaped parentheses in bare destinations, reject invalid labels, incomplete split definitions, or trailing title text instead of silently dropping it, and surface duplicate normalized labels as informational diagnostics. Missing-reference diagnostics are based on parsed inline segments instead of raw source regexes, so collapsed links/images are covered while code spans and non-inline blocks are ignored; shortcut references remain context-aware candidates and only resolve when a matching definition exists.
